...
- **하이브리드 RAG**: 과거 작업 기록을 분석하여 컨텍스트 인식 AI 응답 제공
  - 자연어 쿼리 파싱 ("11월 24일 완료한 작업", "이번 주 통계")
  - 구조화된 쿼리 (날짜, 상태)는 IndexedDB 직접 검색으로 100% 정확도
  - 의미 기반 검색 (벡터 유사도)으로 관련 작업 탐색
- **자동 태그 추천**: 과거 작업 패턴 기반 태그 자동 제안
...
### Testing
- **Vitest 3.1** - 테스트 프레임워크 (494+ tests)
...
### 하이브리드 RAG 시스템
과거 작업 기록을 AI 대화에 컨텍스트로 제공하는 지능형 검색 시스템:
- **쿼리 파싱**: 자연어 ("11월 24일 완료 작업") → 구조화된 검색 조건
- **이중 검색 전략**:
  - 구조화된 쿼리 (날짜, 상태) → IndexedDB 직접 검색 (100% 정확, API 비용 0)
  - 의미 기반 쿼리 ("프로그래밍 작업") → 벡터 유사도 검색 (Orama)
- **자동 태그 추천**: 과거 작업 패턴 + AI 분석으로 태그 제안
- **통계 집계**: 완료율, 날짜별 분포 자동 계산
