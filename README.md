...
```mermaid
graph LR
    TaskComplete --> GoalHandler[목표 진행도]
    GoalHandler --> XPHandler[XP/보상 계산]
    XPHandler --> QuestHandler[퀘스트 갱신]
    QuestHandler --> WaifuHandler[동반자 호감도]
    WaifuHandler --> BlockHandler[타임블록 확정]
```
