...
- `globalGoalRepository.ts` - Global goals (date-independent)
...
- `systemRepository.ts` - System state
...
- **Zustand stores** (`src/shared/stores/`): Global reactive state
  ...
  - `focusStore.ts` - Focus timer state
  - `realityCheckStore.ts` - Reality check state
...
- Each feature module is self-contained with UI components
...
- Styles defined via Tailwind utility classes
...
- **CSS**: Tailwind classes used directly in components (no separate CSS files)
...
11. `globalGoals` - Global goals (primary key: id)
12. `systemState` - System state (primary key: key)
...
- **Version 5**: Added globalGoals table
- **Version 6**: Added systemState table
...
- Version 5 upgrade: Adds globalGoals table
- Version 6 upgrade: Adds systemState table
...
**Hybrid CSS System** (Tailwind CSS + CSS Variables):

1. **`design-system.css`** - Design tokens and scales
   - Typography system: 1.25 ratio scale (12px → 48px)
   - Font families: Pretendard + System fonts
   - Spacing system: 8px base grid
   - Border radius: sm (4px) → 2xl (24px)

2. **`globals.css`** - Theme colors, resets, utilities
   - **Theme System**: Multiple themes (Ocean, Forest, Sunset, etc.)
   - **Color Palette**: CSS custom properties mapped to Tailwind colors
   - **Modal System**: Unified modal styles and animations
   - **Accessibility**: Focus styles, high contrast mode support

3. **`tailwind.css`** - Tailwind directives
   - `@tailwind base`, `components`, `utilities`
   - Base layer overrides using CSS variables
...
### Focus Mode (FocusTimerOverlay)
- **Fullscreen Zen Mode**: Activated via `useFocusStore`.
- **Visual Timer**: Circular progress timer showing remaining time in current block.
- **Task Focus**: Displays top 3 pending tasks for the current block.
- **Block Awareness**: Automatically detects current time block (Morning, Deep Work, etc.).
- **Design**: Dark, high-contrast UI with backdrop blur for immersion.

### Reality Check System
- **Post-Task Feedback**: Modal appears after task completion.
- **Estimation Calibration**: Asks user to compare actual time vs estimated time (Faster/On Time/Slower).
- **Immediate Feedback**: Toast notifications based on response.
- **Purpose**: Helps users improve time estimation skills (metacognition).
