 ...
 *   - 하루 1회 중복 방지 (templateTaskService 연동)
 *
 * @dependencies
 ...
 *   - @/shared/services/template/templateTaskService: 자동생성 상태 관리
 */
 ...
 * 자동 생성 템플릿에서 작업 생성
 *
 * @architecture Option A: Server-First Strategy + 로컬 중복 방지
 *   - 로컬 systemState에서 오늘 이미 실행했는지 먼저 체크 (부팅/데일리 1회)
 *   - Firebase Function이 이미 실행했는지 체크
...
export async function generateTasksFromAutoTemplates(): Promise<void> {
  try {
    ...
    // Step 0: 로컬 중복 방지 - 오늘 이미 실행했는지 체크
    const { hasAutoGeneratedToday, markAutoGenerateComplete } = await import(
      '@/shared/services/template/templateTaskService'
    );
    
    const alreadyRan = await hasAutoGeneratedToday();
    if (alreadyRan) {
      return;
    }
    ...
    if (isFirebaseInitialized()) {
      try {
        ...
        if (lastGenData && lastGenData.date === today && lastGenData.success) {
          // Firebase에서 이미 생성됨 - 로컬 상태도 업데이트
          await markAutoGenerateComplete(lastGenData.generatedCount ?? 0);
          return;
        }
      } catch (firebaseError) {
        console.warn(
    ...
    // Step 3: 로컬 상태 업데이트 (중복 방지용)
    await markAutoGenerateComplete(tasks.length);

    // Step 4: 로컬 생성 완료 상태를 Firebase에 기록
