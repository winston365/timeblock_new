...
interface FocusMusicPlayerProps {
  ...
  onTogglePlay: () => void;
  onNextRandom: (avoidSame?: boolean) => void;
 ...
 * @param props.onFolderChange - 폴더 변경 핸들러
 * @param props.onVolumeChange - 볼륨 변경 핸들러
 * @param props.onTogglePlay - 재생/일시정지 토글 핸들러
 * @param props.onNextRandom - 랜덤 다음 트랙 핸들러
 * @param props.onLoopModeChange - 반복 모드 변경 핸들러
 * @returns 음악 플레이어 UI
 */
    ...
    <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-bg-tertiary)] p-3 shadow-sm max-w-3xl">
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => onLoopModeChange('track')}
          ...
          <button
            onClick={() => onLoopModeChange('folder')}
            className={`rounded-xl border px-3 py-2 text-sm transition ${
              loopMode === 'folder'
                ? 'border-[var(--color-primary)] bg-[var(--color-primary)] text-white shadow-sm'
                : 'border-[var(--color-border)] bg-[var(--color-bg-surface)] text-[var(--color-text-tertiary)] hover:bg-[var(--color-bg-tertiary-hover)]'
