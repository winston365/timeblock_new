...
import { formatBucketRangeLabel, MAX_TASKS_PER_BUCKET } from '../utils/threeHourBucket';
...
export function ThreeHourBucket({
  ...
  const canAcceptMoreFromOtherLocation = tasks.length < MAX_TASKS_PER_BUCKET;
  ...
  const handleInlineInputKeyDown = async (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && inlineInputValue.trim()) {
      ...
      if (tasks.length >= MAX_TASKS_PER_BUCKET) {
        addToast(`이 시간대에는 최대 ${MAX_TASKS_PER_BUCKET}개의 작업만 추가할 수 있습니다.`, 'warning', 3000);
        return;
      }
  ...
  return (
    ...
    >
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="text-xs text-[var(--color-text-tertiary)]">{tasks.length}/{MAX_TASKS_PER_BUCKET}</div>
      </div>
        ...
        {!isLocked && (
          <div className="pt-1">
            <input
              ...
              disabled={tasks.length >= MAX_TASKS_PER_BUCKET}
            />
            {tasks.length >= MAX_TASKS_PER_BUCKET && (
              <div className="mt-2 text-xs text-[var(--color-text-tertiary)]">
                ⚠️ 이 시간대에는 최대 {MAX_TASKS_PER_BUCKET}개까지만 추가할 수 있습니다
              </div>
            )}
          </div>
        )}
