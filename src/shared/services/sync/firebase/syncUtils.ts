 ...
 *   - syncLogRepository: 디바이스 ID 저장 및 로드
 * @note localStorage 대신 Dexie systemState 테이블 사용 (앱 전체 일관성)
 */

import { getDeviceIdFromDb, saveDeviceIdToDb } from '@/data/repositories/syncLogRepository';
import { generateId } from '@/shared/lib/utils';

// ============================================================================
// Device ID Management (Repository-based)
// ============================================================================
 ...
 * Repository에서 deviceId를 로드하거나 새로 생성합니다.
 */
export async function initializeDeviceId(): Promise<string> {
  if (cachedDeviceId) return cachedDeviceId;

  try {
    const storedId = await getDeviceIdFromDb();
    if (storedId) {
      cachedDeviceId = storedId;
      return cachedDeviceId;
    }
  } catch (error) {
    console.warn('Failed to load deviceId:', error);
  }
  ...
  try {
    await saveDeviceIdToDb(cachedDeviceId);
  } catch (error) {
    console.warn('Failed to save deviceId:', error);
  }
